...
**Files:**
- Modify: `simple_bot.py` — новый хелпер; замена в `finalize_order_creation` (~2853), `handle_republish_order` (~4176), два места отмены edit_amount (`on_callback` ~1599 и ~1692 блок `back_to_control`)
...
- [ ] **Step 2: Заменить 4 копии на вызов хелпера**

В каждом из 4 мест заменить литерал `InlineKeyboardMarkup([... remind_bot ... close ...])`, присваиваемый переменной `control_keyboard`, на:

```python
    control_keyboard = build_order_control_keyboard(bid)
```

Места (искать по `callback_data=f"remind_bot:`):
1. `finalize_order_creation` (~2853)
2. `handle_republish_order` (~4176) — там переменная `bid` называется `new_bid`; использовать `build_order_control_keyboard(new_bid)`
3. `on_callback` блок `back_to_control:` (~1599)
4. `on_callback` action `back_to_control` (~1692) — сверить имя `bid` в этой ветке

> Внимание: в местах 3-4 (отмена редактирования суммы) переменная может называться `bid` — сверить по коду и передать правильное имя.
...
> Примечание: `include_back=False` — в шаблонном флоу возврат «Назад» из чатов не нужен (сумма — единственный предыдущий шаг, мерчант просто заново вводит сумму или шлёт /create). Это закрывает камень №5 из спеки: мы не даём кнопку, ведущую на несуществующий шаг ttl.
...
В `main`, рядом с другими `application.add_handler(CommandHandler(...))`, добавить:
